# Vite Skill

This skill summarizes the Vite documentation into actionable guidance for setup, configuration, plugins, HMR, build/deploy, and the Environment API.

## Scope

- Vite project setup and CLI usage
- Core configuration patterns and environment variables
- Plugin authoring and HMR APIs
- Build, deploy, and SSR workflows
- Environment API (instances, plugins, frameworks, runtimes)
- Future deprecations and migration guidance

## References

- Getting started: references/getting-started.md
- Philosophy: references/philosophy.md
- Why Vite: references/why-vite.md
- Features: references/features.md
- CLI: references/cli.md
- Using plugins: references/using-plugins.md
- Dependency pre-bundling: references/dep-pre-bundling.md
- Assets: references/assets.md
- Build: references/build.md
- Static deploy: references/static-deploy.md
- Env and modes: references/env-and-mode.md
- SSR: references/ssr.md
- Backend integration: references/backend-integration.md
- Troubleshooting: references/troubleshooting.md
- Performance: references/performance.md
- Rolldown: references/rolldown.md
- Migration: references/migration.md
- Breaking changes: references/breaking-changes.md
- Plugin API: references/api-plugin.md
- HMR API: references/api-hmr.md
- JavaScript API: references/api-javascript.md
- Config reference: references/config.md
- Environment API intro: references/api-environment.md
- Environment instances: references/api-environment-instances.md
- Environment plugins: references/api-environment-plugins.md
- Environment frameworks: references/api-environment-frameworks.md
- Environment runtimes: references/api-environment-runtimes.md

## Links

- [Documentation](https://vite.dev/)
- [Releases](https://github.com/vitejs/vite/releases)
- [Changelog](https://github.com/vitejs/vite/blob/main/packages/vite/CHANGELOG.md)
- [GitHub](https://github.com/vitejs/vite)
- [npm](https://www.npmjs.com/package/vite)
