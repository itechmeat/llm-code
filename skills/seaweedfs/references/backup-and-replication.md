...
- `4.40` fixes `weed filer.backup` silently losing data when it hit a transient not-found error mid-run; previously such an error could be swallowed and the affected entry skipped rather than retried. Re-audit long-running backup jobs that operated through intermittent storage errors on older releases, since gaps from that failure mode would not have been logged as errors.
...
- Do not assume pre-`4.40` `filer.backup` runs were complete if they crossed a transient not-found error; treat that as a silent-gap risk worth spot-checking against the source namespace.
